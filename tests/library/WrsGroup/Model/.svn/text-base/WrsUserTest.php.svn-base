...
class WrsGroup_Model_WrsUserTest extends ModelTestCase
{
    /**
     * @var WrsGroup_Model_WrsUser
     */
    protected $_user;
    ...
    {
        parent::setUp();
        $this->_user = new WrsGroup_Model_WrsUser(array(
            'username' => 'emorgan',
            'ldapData' => array(
              'accountexpires' =>
                array(
    ...
    public function testGetDisplayNameWhenPresentInLdapData()
    {
        $displayName = $this->_user->getDisplayName();
        $this->assertEquals('Eugene Morgan', $displayName);
    }

    public function testGetDisplayNameWhenNotPresentInLdapData()
    {
        $user = new WrsGroup_Model_WrsUser(array(
            'username' => 'jtest',
        ));
        $this->assertEquals('jtest', $user->getDisplayName());
    }

    public function testGetEmailAddress()
    {
        $emailAddress = $this->_user->getEmailAddress();
        $this->assertType('WrsGroup_Model_EmailAddress', $emailAddress);
        $this->assertEquals(
            strtolower('eugenemorgan@wrsgroup.com'),
            strtolower($emailAddress->address)
        );
    }
    ...
    public function testGetFirstName()
    {
        ...
        $this->assertEquals('Eugene', $firstName);
    }

    public function testGetLastName()
    {
        $lastName = $this->_user->getLastName();
        $this->assertEquals('Morgan', $lastName);
    }

    public function testGetOrganization()
    {
        $org = $this->_user->getOrganization();
        $this->assertEquals('WRS', substr($org, 0, 3));
    }

    public function testGetUsername()
    {
        $username = $this->_user->getUsername();
        $this->assertEquals('emorgan', $username);
    }

    public function testGetUserGroups()
    {
        $groups = $this->_user->getUserGroups();
        $this->assertType('WrsGroup_Model_RecordSet', $groups);
        $itGroup = $groups->findOneBy('group', 'Information Technology');
        $this->assertType('WrsGroup_Model_Group', $itGroup);
    }

    public function testGetUserGroupsWithNoMemberofData()
    {
        $user = new WrsGroup_Model_WrsUser(array(
            'username' => 'emorgan',
            'ldapData' => array(
    ...
    {
        $userType = $this->_user->getUserType();
        $this->assertEquals(WrsGroup_Model_UserAbstract::WRS_GROUP, $userType);
    }

    public function testIsActiveWhenNoLdapData()
    {
        $user = new WrsGroup_Model_WrsUser(array(
            'username' => 'tuser',
        ));
    ...
    public function testIsActiveWhenLdapDataAndActive()
    {
        $active = $this->_user->isActive();
        $this->assertTrue($active);
    }

    public function testIsActiveWhenLdapDataAndNotActive()
    {
        $ldapData = $this->_user->ldapData;
        $ldapData['useraccountcontrol'][0] = '514';
        $this->_user->setLdapData($ldapData);
        $active = $this->_user->isActive();
        $this->assertFalse($active);
    }

    public function testIsMemberOfWhenFalse()
    {
        $this->assertFalse($this->_user->isMemberOf('Customer Service'));
    }

    public function testIsMemberOfWhenTrue()
    {
        $this->assertTrue($this->_user->isMemberOf('Information Technology'));
    }
}
